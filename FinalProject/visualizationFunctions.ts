...
function useStreamlines(randNumArr:number[][]) {
    ...
    for (let i:number = 0; i < texHeight; i++) {
        for (let j:number = 0; j < texWidth; j++) {
            ...
            let count = 1;
            let average = 0;
            for (let k:number = 0; k < streamX.length-1; k++) {
                ...
                if (streamX[k] != -1 && streamY[k] != -1 && streamX[k+1] != -1 && streamY[k+1] != -1 ) {
                    ...
                    let distanceBetweenPoints:number = Math.round(Math.sqrt(Math.pow(point1[0] - point2[0], 2) + Math.pow(point1[1] - point2[1], 2)));

                    // X Coordinate Calculation for the points I will sample
                    let xComPoints:number[] = new Array(distanceBetweenPoints);
                    ...
                    let yComPoints:number[] = new Array(distanceBetweenPoints);
...
function rungeKutta4(x:number, y:number) {
    ...
    // Set up the error
    let error:number[] = Array(2);
    let errorLowest:number = 1;
