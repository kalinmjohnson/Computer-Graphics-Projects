...
import { initFileShaders, perspective, vec2, vec4, flatten, lookAt, rotateX, rotateY, scalem, vec3 } from '../helperfunctions.js';
let gl;
let program;
//uniform locations
...
let uSpecularSampler; //this will be a pointer to our sampler2D
let uNightSampler; //this will be a pointer to our sampler2D
let uCloudSampler; //this will be a pointer to our sampler2D
let uNormalSampler; //this will be a pointer to our sampler2D
// Light Information
let light_color; // and magnitude or brightness
let light_position;
...
let vSpecularExponent; // how shiny is the thing, higher = shinier
...
function makeSquareAndBuffer() {
    let globePoints = []; //empty array to add vertices to for the sphere
    ...
    for (let lat = 0; lat <= Math.PI; lat += step) { //latitude
        for (let lon = 0; lon <= 2 * Math.PI; lon += step) { //longitude
