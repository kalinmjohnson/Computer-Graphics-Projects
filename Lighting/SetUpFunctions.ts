...
export function getSpherePoints(numOfPoints: number): number {
    let spherePoints: number = 0;
...
export function ScenePoints(numOfPoints, originX, originY, radius, boatsidey) {
    ...
    let watersize: number = 1.0;// Below are the different color options
    ...
    let oldX:number = originX + radius * Math.cos(0);
    let oldY:number = originY + radius * Math.sin(0);
    let angle:number;
    let x: number;
    let y: number;
    ...
    radius = 0.003;
    let step:number = (360.0 / numOfPoints)*(Math.PI / 180.0);
    for (let lat:number = 0; lat <= Math.PI+step ; lat += step){ //latitude
        for (let lon:number = 0; lon + step <= 2*Math.PI+step; lon += step){ //longitude
            //triangle 1
